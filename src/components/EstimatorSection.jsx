...
export default function EstimatorSection() {
  ...
  const currentCategory = SERVICES_DATA_ALL.categories.find(
    (c) => c.name === category,
  );
  const currentSubcategory = currentCategory?.subcategories.find(
    (s) => s.name === subcategory,
  );
  const currentMaterial = currentSubcategory?.materials.find(
    (m) => m.name === material,
  );

  const handleCalculate = () => {
    if (currentMaterial && qty) {
      setPrice(currentMaterial.ratePerSqFt * Number(qty));
    }
  };
        ...
        <div className="col-md-8">
          <div className="p-4">
                ...
                >
                  ...
                  {SERVICES_DATA_ALL.serviceMethods
                    .filter(
                      (method) =>
                        !method.applicableTo ||
                        method.applicableTo.includes(category),
                    )
                    .map((method) => (
            ...
            <div className="row">
              ...
              {/* Result */}
              {price !== null && (
                <p className="col alert alert-danger mt-2 me-3 fw-bold fs-5">
                  Estimated Cost: <strong>AED {price}</strong>
                </p>
              )}
            </div>
